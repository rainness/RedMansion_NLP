...
public class Maps {
   ...
   */
  public static <K extends Comparable<? super K>, V> List<Entry<K, V>> sortedEntries(Collection<Entry<K, V>> entries) {
    List<Entry<K,V>> entriesList = new ArrayList<Entry<K, V>>(entries);
    Collections.sort(entriesList, new Comparator<Entry<K, V>>() {
   ...
   */
  public static <K extends Comparable<? super K>, V> List<Entry<K, V>> sortedEntries(Map<K, V> map) {
  ...
  public static <K, V1, V2> void addAll(Map<K, V1> to, Map<K, V2> from, Function<V2, V1> function) {
    for (Entry<K, V2> entry : from.entrySet()) {
