...
public class HashableCoreMap extends ArrayCoreMap {
  ...
  private final Set<Class<? extends Key<?>>> immutableKeys;
  ...
  public HashableCoreMap(Map<Class<? extends Key<?>>,Object> hashkey) {
    int keyHashcode = 0;
    int valueHashcode = 0;
    
    for (Map.Entry<Class<? extends Key<?>>,Object> entry : hashkey.entrySet()) {
   ...
   */
  @SuppressWarnings("unchecked")
  public HashableCoreMap(ArrayCoreMap other, Set<Class<? extends Key<?>>> hashkey) {
    super(other);
    
    int keyHashcode = 0;
    int valueHashcode = 0;
    
    for (Class<? extends Key<?>> key : hashkey) {
      // NB it is important to compose these hashcodes in an order-independent
      // way, so we just add them all here.
  ...
  public boolean equals(Object o) {
    if (o instanceof HashableCoreMap) {
      ...
      for (Class<? extends Key<?>> key : immutableKeys) {
