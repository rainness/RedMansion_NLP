  ...
  public Set<Entry<K,V>> entrySet() {
    return new AbstractSet<Entry<K,V>>() {
      ...
      public Iterator<Entry<K,V>> iterator() {
        Filter<Entry<K,V>> filter1 = new Filter<Entry<K,V>>() {
          private static final long serialVersionUID = 1L;

          // only accepts stuff not overwritten by deltaMap
          public boolean accept(Entry<K,V> e) {
            K key = e.getKey();
            return ! deltaMap.containsKey(key);
          }
        };

        Iterator<Entry<K, V>> iter1 = new FilteredIterator<Entry<K, V>>(originalMap.entrySet().iterator(), filter1);

        Filter<Entry<K,V>> filter2 = new Filter<Entry<K,V>>() {
          private static final long serialVersionUID = 1L;
          // only accepts stuff not overwritten by deltaMap
          public boolean accept(Entry<K,V> e) {
        ...
        class NullingIterator<K, V> implements Iterator<Entry<K,V>> {
          private Iterator<Entry<K, V>> i;

          public NullingIterator(Iterator<Entry<K, V>> i) {
            this.i = i;
          }
